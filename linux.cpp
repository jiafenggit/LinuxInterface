 ...
 */
#include "linux.h"
void xlm::setMousePos(unsigned int y,unsigned int x)
{
	printf("\033[%d;%dH\033[0m",y,x);
}
void xlm::clearScreen()                       //清屏操作
{
	printf("\033[2J");
}
int xlm::GetKeyWord()
...
bool xlm::Sec_sleep(int s)
{
	...
	request.tv_sec+=s;
	int res=clock_nanosleep(CLOCK_MONOTONIC,TIMER_ABSTIME,&request,&remain);
	if(res!=0)
	{
...
bool xlm::Ms_sleep(int ms)
{
	...
	int s=clock_nanosleep(CLOCK_MONOTONIC,TIMER_ABSTIME,&request,NULL);
	if(s!=0)
	{
		if(s==EINTR)
...
void xlm::QnxTimeToFileTime1(timespec *t, LPFILETIME pft)
{
	long long ll = (long long)t->tv_sec * 10000000 + t->tv_nsec / 100 + 116444736000000000LL;
	pft->dwLowDateTime = (DWORD) ll;
	pft->dwHighDateTime = ll >> 32;
}
void xlm::FileTimeToQnxTime1(LPFILETIME pft, timespec *t)
{
	long long ll = (long long)pft->dwHighDateTime << 32;
	ll += pft->dwLowDateTime;
	ll -= 116444736000000000LL;
	t->tv_sec = (time_t)(ll / 10000000);
	t->tv_nsec = (long)(ll - t->tv_sec * 10000000) * 100;
}

void xlm::TimetToFileTime1( time_t t, LPFILETIME pft )
{
    long long ll = (long long)t * 10000000 + 116444736000000000LL;

    pft->dwLowDateTime = (DWORD) ll;
    pft->dwHighDateTime = ll >>32;
}

time_t xlm::FileTimeToTimet1( LPFILETIME pft )
{
	long long ll = (long long)pft->dwHighDateTime << 32;
    ll += pft->dwLowDateTime;
    ll -= 116444736000000000LL;
...
bool xlm::setBiosTime(struct tm * tParam)
{
	/*
	 * 	struct tm tval;
	...
	tv.tv_sec=mktime(tParam);
	fd = open("/dev/rtc",O_RDONLY);
	if (fd >= 0)
	{
		memcpy(&time,tParam,sizeof(struct rtc_time));
...
DWORD  xlm::Rand_value(DWORD range)
{
...
bool xlm::RegisteSignal(int sig,void (*handler)(int _sig))
{
	struct sigaction sa;
...
int xlm::Bitap(const char *text, const char *find)  //0~
{
...
int xlm::CreateThread(void *(*Dealfunction) (void *),void * aParm,int priority)
{
	pthread_t t;
	pthread_attr_t attr;
	...
	pthread_attr_setschedpolicy(&attr,SCHED_FIFO);                         //set thread scheduler
...
int xlm::ReplaceStr(char * sSrc,const char * sMatchStr,const char * sReplaceStr)
{
	int StringLen;
...
int xlm::GetPrivateProfileInt(const char *key,const char * cField,int nDefault,char *filename)
{
	int nRet=nDefault;
...
int xlm::GetPrivateProfileString(const char* key,const char *cField,char * cDefault,char * cRet,int dRetLen,char * filename )
{
	char str[1024],cTmp[200],*pc=NULL;
	FILE* fp=NULL;
	int len=0;
...
///////////////////////////////////////////////////////////////////
bool xlm::WriteLog(char * msg,int msglen)
{
	char * filename=(char*)"Log.txt";
	FILE* file=fopen(filename,"aw+");
	if(!file)
	{
		fprintf(stderr,"fopen %s was failed:%s\n",filename,strerror(errno));
		fclose(file);
		return false;
	}
	int res=fwrite(msg,msglen,1,file);
	if(res==-1)
	{
		fprintf(stderr,"fwrite %s was failed:%s\n",filename,strerror(errno));
		fclose(file);
		return false;
	}
	fclose(file);
...
int xlm::GetFileSize(char * path,DWORD *len)
{
	int filesize=-1;
...
int FindKey(const char *cKey,char * filename)
{
	...
	nLen=xlm::GetFileSize(filename,&nhLen);
	if(nLen<=0)
	{
		fclose(fp);
		return nFind;
...
///////////////////////////////////////////////////////////////////
bool xlm::WritePrivateProfileInt(const char * key,const char *cField,int nValue,char * filename)
{
	FILE* fp=NULL;
...
bool xlm::WritePrivateProfileString(const char * key,const char *cField,char *cValue,char * filename)
{
	FILE* fp=NULL;
	int len,newlen,hinewlen;
	char str[1024],cNewStr[200],cTmp[100],*pc=NULL;
...
bool xlm::getLocalActiveNetworkInfo(char * res_name)
{
	struct ifreq ifr;
	struct ifconf ifc;
	char buf[2048]={0};
...
bool xlm::getLocalALLNetworkInfo(char * ips)
{
...
int  xlm::creatbindTcpsock(char * bindip,int bindport)
{
...
int  xlm::creatbindUdpsock(char * bindip,int bindport)
{
...
bool xlm::setaddrReuse(int sock)
{
	const int opt=1;
...
bool xlm::setsockNonblock(int sock)
{
	unsigned long opt=1;
...
bool xlm::setsockBroadcast(int sock)
{
	const int opt=1;
...
bool xlm::setsockRwbuf(int sock,int rwval)
{
...
bool xlm::setsockTimeout(int sock,int tsval,int tusval)
{
	struct timeval tv;
	memset(&tv,0,sizeof(struct timeval));
	tv.tv_sec=tsval;
...
bool set_speed(int fd, int speed)
{
	int speed_arr[] = { B38400, B19200, B9600, B4800, B2400, B1800,B1200, B300, B200, B150};
	int name_arr[] =  {38400,  19200,  9600,  4800,  2400,  1800, 1200,  300, 200, 150 };
	int   i,status;
	struct termios   Opt;
	memset(&Opt,0,sizeof(struct termios));
	if(tcgetattr(fd, &Opt)!=0)
	{
		fprintf(stderr,"set_speed tcgetattr was failed:%s\n",strerror(errno));
		return false;
	}
	for ( i= 0;  i < (int)(sizeof(speed_arr)/sizeof(int));  ++i)
	{
		if(speed == *(name_arr+i))
		{
			...
			if  (status != 0)
			{
				fprintf(stderr,"tcsetattr fd failed:%s %d fd=:%d\n",strerror(errno),*(speed_arr+i),fd);
				return false;
			}
			tcflush(fd,TCIOFLUSH);
			break;
 ...
 */
int xlm::InitSerialPort(int port,int baudrate,int blockflag,char checkflag,int databyte,int stopbyte)
{
	int fd;
	char devpath[20]={0};
	memset(devpath,0,20);
	...
	{
		perror("GetSerial options failed");
		return -1;
	}
	...
	switch (databyte) /*设置数据位数*/
	{
		...
		case 8:
			...
			break;
		default:
			fprintf(stderr,"unsupported set data byte\n"); return false;
	}
	switch(checkflag)
	{
		case 'n':                    //无校验
	...
	switch (stopbyte)
	{
		...
		case 2:
		   ...
		   break;
		default:
			 fprintf(stderr,"unsupported stop byte\n");
			 return -1;
	}
	...
	if(blockflag)
	{
		 ...
		 * VTIME >  0, VMIN >  0
		 */
		options.c_cc[VTIME] = 10;        /* 设置超时5 seconds                  */
	...
	{
		perror("SetupSerial Effective was failed!");
		return -1;
	}
	return fd;
}

void xlm::showByte8(unsigned char ch)
{
	for(int i=7;i>=0;--i)
	{
		if(ch&(1<<i))
...
void xlm::clrAllbyte8(unsigned char * ch)
{
	*ch='\0';
}
void xlm::setByte8(unsigned char * ch,unsigned int index)
{
	*ch|=(1<<index);
}
void xlm::clrByte8(unsigned char * ch,unsigned int index)
{
	*ch&=~(1<<index);
}
int  xlm::calc_1_Num(int _n)
{
	int num=0;
	while(_n)
	{
...
unsigned char xlm::pciRead8(unsigned char bus, unsigned char dev, unsigned char fnc, unsigned char reg)
{
...
unsigned short xlm::pciRead16(unsigned char bus, unsigned char dev, unsigned char fnc, unsigned char reg)
{
    outl((1 << 31) + (bus << 16) + ((dev & 0x1f) << 11) +\
            ((fnc & 0x07) << 8) + (reg & ~(0x03)), PCI_INDEX);
...
unsigned int xlm::pciRead32(unsigned char bus, unsigned char dev, unsigned char fnc, unsigned char reg)
{
    outl((1 << 31) + (bus << 16) + ((dev & 0x1f) << 11) +\
            ((fnc & 0x07) << 8) + (reg & ~(0x03)), PCI_INDEX);
    return inl(PCI_DATA);
...
void xlm::pciWrite8(unsigned char bus, unsigned char dev, unsigned char fnc, unsigned char reg, unsigned char value)
{
    outl((1 << 31) + (bus << 16) + ((dev & 0x1f) << 11) +\
            ((fnc & 0x07) << 8) + (reg & ~(0x03)), PCI_INDEX);
...
void xlm::pciWrite16(unsigned char bus, unsigned char dev, unsigned char fnc, unsigned char reg, unsigned short value)
{
    outl((1 << 31) + (bus << 16) + ((dev & 0x1f) << 11) +\
            ((fnc & 0x07) << 8) + (reg & ~(0x03)), PCI_INDEX);
...
void xlm::pciWrite32(unsigned char bus, unsigned char dev, unsigned char fnc, unsigned char reg, unsigned int value)
{
    outl((1 << 31) + (bus << 16) + ((dev & 0x1f) << 11) +\
            ((fnc & 0x07) << 8) + (reg & ~(0x03)), PCI_INDEX);
    outl(value, PCI_DATA);
...
/*
 *resource2这个是和可映射地址对应的，就是使用lspci -v 查看到的结果Memory at
 */
void *xlm::getMapAddrPci()
{
	char *filename=(char*)"/sys/devices/pci0000:00/pci_bus/0000:00/subsystem/0000:05/device/0000:05:04.0/resource2";
	...
	void *map_base = mmap(NULL, 250000, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);      //映射一个页大小
	if(map_base == MAP_FAILED)
	{
		fprintf(stderr,"+map a page was failed:%s fd:%d\n",strerror(errno),fd);
		close(fd);
		return (void*)NULL;
	}
...
WORD xlm::CalCheckCRC(unsigned char* lpBuf,int iLen)
{
	 int i,ch;
	 WORD wCRC=0xffff;
	 for(i=0;i<iLen;i++)
