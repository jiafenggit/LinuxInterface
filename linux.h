...
#pragma pack(push,1)       //单字节对齐使用方法双#pragma
typedef struct
{
	DWORD dwLowDateTime;   /* low 32 bits  */
	DWORD dwHighDateTime;  /* high 32 bits */
} FILETIME;                //100ns //before 1970 116444736000000000 * 100ns
#pragma pack(pop)
...
namespace xlm                                                                               	//自定义命名空间
{
	...
	DWORD  Rand_value(DWORD range);                                                             //产生随机数随机度较高
	...
	int  CreateThread(void *(*Dealfunction)(void *),void * aParm,int priority);                 //建立一个分离线程需要sudo
	WORD CalCheckCRC(unsigned char* lpBuf,int iLen);                                            //CRC校验以WORD为单位
	/*时间相关的函数*/
	void QnxTimeToFileTime1(timespec *t, LPFILETIME pft);
	void FileTimeToQnxTime1(LPFILETIME pft, timespec *t);
	void TimetToFileTime1( time_t t, LPFILETIME pft );
	time_t FileTimeToTimet1( LPFILETIME pft );
	bool setBiosTime(struct tm *);                                                              //设置系统bios时间
	...
	/*配置文件接口*/
	bool WriteLog(char * msg,int msglen);                                                       //打印日志文件n是msg的实际长度
	...
	/*linux pci相关操作,先在配置空间获取可映射的地址，然后找到对应的resource{0,1,2..}进行mmap映射*/
	//配置空间读写函数reg 对应的地址寄存器
	#define PCI_INDEX 0xcf8
	#define PCI_DATA  0xcfc
	unsigned char pciRead8(unsigned char bus, unsigned char dev, unsigned char fnc, unsigned char reg);
	unsigned short pciRead16(unsigned char bus, unsigned char dev, unsigned char fnc, unsigned char reg);
	unsigned int pciRead32(unsigned char bus, unsigned char dev, unsigned char fnc, unsigned char reg);
	void pciWrite8(unsigned char bus, unsigned char dev, unsigned char fnc, unsigned char reg, unsigned char value);
	void pciWrite16(unsigned char bus, unsigned char dev, unsigned char fnc, unsigned char reg, unsigned short value);
	void pciWrite32(unsigned char bus, unsigned char dev, unsigned char fnc, unsigned char reg, unsigned int value);
	void *getMapAddrPci();
	/***************************************************************************************************************/
